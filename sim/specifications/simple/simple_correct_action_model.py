...
def KinPickPred(param, hyp):
  if sum( param['q'], negate(param['p']) ) == (0,1):
    return [('kinPick', True)]
  return [('kinPick', False)]

# -----------------------------

def UnderPred(param, hyp):
  if not hyp[ str(param['arm']) + ".armEmpty" ]:
    return [ ('1.under', (param['q1'], sum(param['q1'],(0,-1))) ), ('2.under', (param['q2'], sum(param['q2'],(0,-1))) ) ]
  return []

# -----------------------------

def KinMovePred(param, hyp):
  # checking translatory kinematic feasibility
  if not ( sum( param['q2'], negate(param['q1']) ) in ( (0,1), (0,-1), (1,0), (-1,0) ) ):
    return [('kinMove', False)]
...
pick_rule2_precond = [  ( 'fromPred', 'arm', 'armEmpty', True ), ( 'fromParam', 'arm', 'pos', 'q' ), ( 'fromParam', 'b', 'pos', 'p' ), ('fromFunc', KinPick, True ), ('fromPred', 'b', 'movable', False ) ]
